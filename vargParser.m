...
% 
    ...
    if mod(length(vargsin),2)
        error('Incorrect number of fields in user supplied arguments, must be a multiple of 2 (Specifier + Value)');
    elseif mod(length(vdefaults),3)
        error('Incorrect number of fields in defaults, must be a multiple of 3 (Specifier + Value + Valid Values)');
    end
